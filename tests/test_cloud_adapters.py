...
def test_cloud_build_uses_provenance_and_dedicated_identity() -> None:
    spec = build_spec(BuildRequest(
        project="project", region="europe-west1", source_bucket="artifacts",
        source_object="run/source.tar.gz", image_uri="registry/image:attempt-1",
...
def test_cloud_run_job_has_hard_limits_and_no_platform_retry() -> None:
    spec = job_spec(JobRequest(
        project="project", region="europe-west1", job_name="attempt-1",
        image_uri="registry/image@sha256:abc", service_account="runner@project.iam.gserviceaccount.com",
        replication_id="replication", attempt_id="attempt", output_uri="gs://bucket/run/attempt",
...
def test_execution_api_uses_regional_build_endpoint() -> None:
    ...
    operation = api.submit_build(BuildRequest(
        project="project", region="europe-west1", source_bucket="artifacts",
        source_object="run/source.tar.gz", image_uri="registry/image:attempt",
        service_account="builder@project.iam.gserviceaccount.com",
    ))
