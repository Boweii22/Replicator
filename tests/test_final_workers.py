...
async def _scenario() -> None:
    ...
    replication = Replication(
        ...
        title="Worker chain",
        status=ReplicationStatus.VERIFYING,
    )
    ...
    metrics_uri = store.put_bytes(
        f"{replication.id}/metrics.json", json.dumps({claim.id: 10.1}).encode(), "application/json"
    )
    ...
    await verifier.handle(
        WorkMessage(
            event_type="verify.requested", replication_id=replication.id, attempt_id=attempt.id
        )
    )
    await bus.drain()
