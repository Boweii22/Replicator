from packages.schemas.models import Attempt, Claim, VerdictStatus, VisionAssessment
...
def test_visual_assessment_requires_specific_evidence() -> None:
    assessment = VisionAssessment(
        same_trend=True,
        same_series_ordering=True,
        comparable_scale=True,
        same_scientific_conclusion=True,
    ...
    claim = Claim(
        ...
        text="same curve",
        claim_type="figure",
        figure_gcs_uri="gs://evidence/paper.png",
        feasible=True,
    )
    verdict = verify_figure_claim(
        claim, Attempt(plan_id="plan", n=1), "gs://evidence/ours.png", 0.9, assessment
    )
